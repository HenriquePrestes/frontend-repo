...
export default function AppointmentCard({ appointment, onCancel }) {
  ...
  const displayValue = (val) => {
    ...
    if (typeof val === 'object') {
      return (
        val.nome || val.name || val.fullName ||
        (val.firstName && val.lastName ? `${val.firstName} ${val.lastName}` : null) ||
        ''
      );
    }
    return String(val);
  ...
  const nameText = displayValue(name) || displayValue(appointment.paciente) || '';
  const specialtyText = displayValue(specialty) || displayValue(appointment.medico) || '';
  const crmText = displayValue(crm) || (appointment.medico && (appointment.medico.crm || appointment.medico.crmNumber)) || '';
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div>
          <h3 className="font-semibold">{nameText}</h3>
          <p className="text-sm text-slate-600">{specialtyText} {crmText ? '• ' + crmText : ''}</p>
        </div>
      </div>
